                ...
                using (MySqlConnection connection = new MySqlConnection(Connection))
                {
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                Database.Query($"update `accounts` set `redbucks`=`redbucks`+{reds} where `login`='{name}'");
                            }
                            else
                            {
                                lock (Main.Players)
                                ...
                                NAPI.Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        Plugins.Notice.Send(client, Plugins.TypeNotice.Success, Plugins.PositionNotice.TopCenter, $"Вам пришли {reds} Redbucks", 3000);
                    ...
                    case Type.Character:
                        {
                            if (acc.RedBucks < 100)
                            {
                                Plugins.Notice.Send(client, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, "Недостаточно Redbucks!", 3000);
                                return;
                            }
                    ...
                    case Type.Slot:
                        {
                            ...
                            Database.Query($"update `accounts` set `redbucks`={Main.Accounts[client].RedBucks} where `login`='{Main.Accounts[client].Login}'");
                            return;
                        }
                }
