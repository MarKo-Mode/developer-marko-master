...
namespace NeptuneEvo.Working
{
    class WorkManager : Script
    {
        ...
        public static void JobJoin(Player player, int job)
        {
            ...
            {
                Plugins.Notice.Send(player, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы не можете устроиться на работу, потому что состоите в организации", 3000);
                return;
            }
            if (Main.Players[player].WorkID != 0)
            {
