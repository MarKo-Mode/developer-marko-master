...
namespace NeptuneEvo.Working
{
    class Construction : Script
    {
        ...
        public static void JobJoin(Player player)
        {
            ...
            if (Main.Players[player].WorkID != 0)
            {
                Plugins.Notice.Send(player, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы уже работаете: {Working.WorkManager.JobStats[Main.Players[player].WorkID - 1]}", 3000);
                return;
            }
