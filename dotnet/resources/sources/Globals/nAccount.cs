...
namespace NeptuneEvo.Globals.nAccount
{
    public class Account : AccountData
    {
        ...
        public async Task<RegisterEvent> Register(Player client, string login_, string pass_, string email_, string promo_)
        {
            try
            {
                ...
                await Database.QueryAsync($"INSERT INTO `accounts` (`login`,`email`,`password`,`hwid`,`ip`,`socialclub`,`redbucks`,`viplvl`,`vipdate`,`promocodes`,`character1`,`character2`,`character3`) " +
        ...
        public async Task DeleteCharacter(Player player, int slot, string firstName_, string lastName_, string password_)
        {
            ...
            Plugins.Notice.Send(player, Plugins.TypeNotice.Success, Plugins.PositionNotice.TopCenter, $"Персонаж {firstName} {lastName} успешно удален", 3000);
