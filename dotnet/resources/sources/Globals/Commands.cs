    ...
    class Commands : Script
    {
        ...
        public static void CMD_addPromo(Player player, int uuid, string promocode)
        {
            try
            {
                ...
                Plugins.Notice.Send(player, Plugins.TypeNotice.Success, Plugins.PositionNotice.TopCenter, $"success", 3000);
            }
            catch { }
        }
        ...
        public static void CMD_checkDim(Player player, int id)
        {
            try
            {
                ...
                Plugins.Notice.Send(player, Plugins.TypeNotice.Info, Plugins.PositionNotice.TopCenter, $"Измерение игрока - {target.Dimension.ToString()}", 4000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_showPlayerStats(Player admin, int id)
        {
            try
            {
                ...
                {
                    Plugins.Notice.Send(admin, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Игрок с таким ID не найден", 3000);
                    return;
                }
        ...
        public static void ACMD_aclear(Player player, string target)
        {
            try
            {
                ...
                Database.Query($"UPDATE `characters` SET `money`=0,`fraction`=0,`fractionlvl`=0,`hotel`=-1,`hotelleft`=0,`sim`=-1,`PetName`='null' WHERE firstname='{split[0]}' AND lastname='{split[1]}'");
                // CLEAR BANK MONEY
        ...
        public static void CMD_offredbaks(Player client, string name, long amount)
        {
            ...
            try
            {
                ...
                Database.Query($"update `accounts` set `redbucks`=`redbucks`+{amount} where `login`='{name}'");
        ...
        public static void CMD_report(Player player, string message)
        {
            try
            {
                ...
                Plugins.Notice.Send(player, Plugins.TypeNotice.Info, Plugins.PositionNotice.TopCenter, $"Вы отправили жалобу: {message}", 3000);
