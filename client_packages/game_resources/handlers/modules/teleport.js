...
mp.events.add('MarKoToggle', () => {
    MarKoToggle = !MarKoToggle;
    if (MarKoToggle) status=`~h~~g~Включен.`; else status=`~h~~r~Выключен.`;
...
mp.events.add('render', () => {
    if(waypoint !== mp.game.invoke('0x1DD1F58F493F1DA5')){
        ...
        if (waypoint) {
            ...
            if (MarKoToggle) mp.events.call('tpToWaypoint');
        }  
    }
});
